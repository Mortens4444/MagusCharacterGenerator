...
/// <summary>
/// Hő Teremtése (p. 293-294). Raises temperature by 10°C per E, costing 2 Mp per E. Damage
...
/// </summary>
public sealed class HeatCreation : IElementCreationMosaic
{
    private const int DamageThresholdCelsius = 100;

    public string Name => "Heat Creation";

    public int CastingTimeInSegments => 2;

    public int GetManaCost(int strength) => Validate(strength) * 2;

    public int GetTemperatureIncrease(int strength) => Validate(strength) * 10;

    public CreatedElement Create(int strength)
    {
        Validate(strength);
        return new CreatedElement { ParaElement = ParaElementType.Heat, Strength = strength };
    }

    /// <summary>2 Sp once the resulting temperature reaches 100°C, +2 Sp per further 10°C.</summary>
    public int GetDamage(int resultingTemperatureCelsius) =>
        resultingTemperatureCelsius >= DamageThresholdCelsius
            ? 2 + (resultingTemperatureCelsius - DamageThresholdCelsius) / 10 * 2
            : 0;
