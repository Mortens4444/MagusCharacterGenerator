...
namespace MAGUS.Classes.NonPlayableCharacters.Fighter;

public class BlackHeavyInfantry : Class, IClass, IJustFight
{
    public BlackHeavyInfantry() : base(1, false) { }

    public BlackHeavyInfantry(int level, bool autoGenerateSkills) : base(level, autoGenerateSkills) { }
    ...
    public override string Name => "Black Heavy Infantry";

    public override List<Thing> StartingEquipment =>
    [
        GetArmor()
        // Talisman of the priests of Bul Ruurig
        // Engraving capable of storing elemental force eruptions
        // Runic Rod of Stone Projection
    ];
    ...
    public static Chainmail GetArmor()
    {
        var roll = new DiceThrow().Throw(ThrowType._1D100);
        return roll switch
        {
            <= 25 => new BlueLunirArmor(),
            <= 65 => new RedLunirArmor(),
            <= 85 => new GreenLunirArmor(),
            <= 97 => new BlackLunirArmor(),
            _ => new MoonLunirArmor()
        };
    }
}
