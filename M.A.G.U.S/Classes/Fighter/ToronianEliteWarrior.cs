...
public class ToronianEliteWarrior : Class, IClass, IJustFight
{
    public ToronianEliteWarrior() : base(1, false) { }

    public ToronianEliteWarrior(int level, bool autoGenerateSkills) : base(level, autoGenerateSkills) { }
    ...
    public override string Name => "Toronian Elite Warrior";
    ...
    [
        new MagicLore(level: 2) { Note = "Warlock" },
        new Healing(level: 3),
        new Backstab(level: 4),
        new WeaponUse(QualificationLevel.Master, 5),
        new PoisoningAndNeutralization(level: 6),
        new EscapeBonds(level: 7),
        new CamouflageOrDisguise(level: 8)
    ]);
