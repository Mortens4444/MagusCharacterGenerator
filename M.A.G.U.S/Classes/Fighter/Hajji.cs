...
public class Hajji : Class, IClass, IJustFight
{
    public Hajji() : base(1, false) { }

    public Hajji(int level, bool autoGenerateSkills) : base(level, autoGenerateSkills) { }
    ...
    [
        ...
        new WeaponUse(),
        new WeaponUse(),
        new Riding() { Note = "Camel or horse" },
        new Running(),
        new Fistfight(),
        new DesertSurvival()
    ]);
    ...
    [
        new Bloodlust(level: 2),
        new DesertSurvival(QualificationLevel.Master, 4),
        new TrackingConcealment(level: 6) { Note = "In desert" },
    ]);
